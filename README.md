...
XXXX is a prototype showing how vCPUs of a given VM can be exposed to different performance levels by dynamically managing the underlying pinning.
It is composed of a local scheduler, deployed on each server, which manage deployments requests and monitor the usage.
...
git clone https://github.com/jacquetpi/XXXX
cd XXXX/
cp dotenv .env
python3 -m venv venv
...
The workload trace is an heavy file. We provide an example [hosted separately](TODO) that should be placed in ```debug/``` folder

- Offline execution

Your oversubscription template must be set to its default value ```OVSB_TEMPLATE="H,M,L"``` inside your ```.env``` file to run this trace
> H: High perf
> M: Mid perf
> L: Low perf
...
```bash
source venv/bin/activate
tar -xzvf debug/XXX.tar.gz -C debug/
python3 -m schedulerlocal --topology=debug/topology_EPYC-7662-exp.json --load=debug/XXX.csv --debug=1
```
